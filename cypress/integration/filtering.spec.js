	...
	it("can use all filters", () => {
		cy.document().then((doc) => {
			...
			for (let i = 0; i < filterLen; i++) {
				...
				cy.get("#allProductsWrapper")
					.find(".product")
					.its("length")
					.should("lt", defaultProductLen);
	...
	it("can use filters in combination", () => {
		cy.document().then((doc) => {
			...
			for (let i = 0; i < filterLen - 1; i++) {
				...
				const currentProductLen =
					doc.querySelectorAll(".product").length;
				// Check if length is reduce with doubling up of filters
				cy.get(`#filter${i + 1}`).select(1);
				cy.get("#allProductsWrapper")
					.find(".product")
					.its("length")
					.should("lt", currentProductLen);
